...
def max_vote(df, threshold):
    new = pd.DataFrame({"target":df["target"].values})
    for name in df.columns[1:8]:
        new[name] = (df[name]>threshold).astype(int)
    new["pred"] = (new.iloc[:,1:8].sum(axis=1)>3).astype(int)
    print("threshold {0} is {1}".format(threshold, metrics.f1_score(new["target"], new["pred"])))
    return new["pred"]

for thresh in np.arange(0.1, 0.501, 0.01):
    thresh = np.round(thresh, 2)
    max_vote(xg_val, thresh)


pred_max_y = max_vote(xg_test, 0.33)
out_df = pd.DataFrame({"qid":test_df["qid"].values})
out_df['prediction'] = pred_max_y
