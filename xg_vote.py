...
## XG boost
xg_train_X, xg_train_y = xg_train.iloc[:,1:], xg_train.iloc[:,0]
xg_val_X, xg_val_y = xg_val.iloc[:,1:], xg_val.iloc[:,0]
xg_test_X, xg_test_y = xg_test.iloc[:,1:], xg_test.iloc[:,0]

dtrain = xgb.DMatrix(xg_train_X, label=xg_train_y)
dval = xgb.DMatrix(xg_val_X, label=xg_val_y)
dtest = xgb.DMatrix(xg_test_X, label=xg_test_y)

# set xgboost params
param = {
    'max_depth': 3,  # the maximum depth of each tree
    'n_estimators':200,  # the maximum depth of each tree
    'eta': 0.3,  # the training step for each iteration
    'silent': 1,  # logging mode - quiet
    'objective': 'binary:logistic',  # error evaluation for binary class
    }
...
for thresh in np.arange(0.1, 0.501, 0.01):
    ...
    print("F1 score at threshold {0} is {1}".format(thresh, metrics.f1_score(xg_val_y.values, (pred_val>thresh).astype(int))))


pred_test_y = (pred_test>0.35).astype(int)
